...
```{r cars_per_day}
#' cars_per_day: Simulate daily traffic at an intersection
#'
#' Simulates the total number of cars passing through an intersection per day.
#' The day is divided into 5 time blocks with different traffic distributions:
#' \itemize{
#'   \item Midnight–7am (8 hours): Poisson(1) cars per hour
#'   \item 8am rush (1 hour): Normal(mean = 60, var = 12)
#'   \item 9am–4pm (8 hours): Poisson(8) cars per hour
#'   \item 5pm rush (1 hour): Normal(mean = 60, var = 12)
#'   \item 6pm–11pm (6 hours): Poisson(12) cars per hour
#' }
#' @param seed Optional integer. If provided, sets the random number generator
#'        seed for reproducibility.
#' @return A numeric value giving the total number of cars for that day.
...
```{r cars_per_day_sim}
#' cars_per_day_sim: Monte Carlo estimate of daily traffic
#'
#' Estimates the expected number of cars passing through an intersection
#' in one day using Monte Carlo simulation over many simulated days.
#'
#' The day is divided into 5 traffic blocks with different distributions:
#' \itemize{
#'   \item Midnight–7am (8 hours): Poisson(1) cars per hour
#'   \item 8am rush (1 hour): Normal(mean = 60, var = 12)
#'   \item 9am–4pm (8 hours): Poisson(8) cars per hour
...
#' }
#'
#' @param days Integer. Number of simulated days (default = 100000).
#' @param seed Optional integer. If provided, sets the random number generator
#'        seed for reproducibility.
#' @return A numeric value: the Monte Carlo estimate of the expected
#'   number of cars per day.
  
cars_per_day_sim <- function(days = 100000, seed = NULL) {
  if (!is.null(seed)) set.seed(seed)
...
cars_per_day_sim(days = 100000)
```

```{r compare_simulation_time}
microbenchmark(
  per_day_simulation <- mean(replicate(100000, cars_per_day())),
  one_time_simulation <- cars_per_day_sim(days = 100000),
  times = 10, 
  unit = "ms"
)
```
...
I solved this problem in two different ways: first by simulating traffic block by block for a single day and repeating many times, and second by using a fully vectorized approach to simulate many days at once. Based on a Monte Carlo simulation of 100,000 independent days, the estimated average number of cars that pass the intersection per day is about 264.

Both results align with the theoretical expected value of 264.

$$
\mathbb{E}[\text{total}] = 8 \cdot 1 + 1 \cdot 60 + 8 \cdot 8 + 1 \cdot 60 + 6 \cdot 12 = 264
$$

But the vectorized version was much faster. A benchmark showed that the per-day simulation took about 330 ms on average, whereas the one-time vectorized simulation took only about 59 ms, making the vectorized method nearly six times faster.
...
Based on the histograms and the summary statistics, we can see distribution of view counts is heavily right-skewed, with some very popular ads with huge view counts.
...
Based on the histograms and the summary statistics, we can see distribution of like counts is heavily right-skewed, with some very popular ads with huge like counts.
...
Based on the histograms and the summary statistics, we can see distribution of dislike counts is also highly right skewed, which is similar to the previous two variables, but much smaller values overall.
...
Based on the histograms and the summary statistics, we can see distribution of favorite counts is flat at zero, indicating no variation.
...
Based on the histograms and the summary statistics, we can see distribution of comment counts is heavily right-skewed, with most ads having few comments.

Therefore, `favorite_count` is inappropriate to use as the outcome in a linear regression model. Other four variables, `view_count`, `like_count`, `dislike_count`, `comment_count` are use as the outcome in a linear regression model, but they require a transformation to handle skewness. I applied the log transformation and used `log1p()` to compute $log(1+x)$ since count can be 0.
...
```{r fit_regression_models}
# define the outcome variables to model
outcomes <- c("log_view_count", "log_like_count", 
              "log_dislike_count", "log_comment_count")

# define the common set of predictors (right-hand side of formula)
formula_rhs <- "funny + show_product_quickly + patriotic + 
                celebrity + danger + animals + use_sex + year"

# for each outcome, build a regression formula and fit a linear model
models <- lapply(outcomes, function(y) {
  reg_formula <- as.formula(paste(y, "~", formula_rhs))
  lm(reg_formula, data = youtube_clean)
})

# name each fitted model according to its outcome
...
I used ChatGPT to assist with writing function documentation for Problem 2, specifically to describe the traffic distributions. It also helped me revise the code for running multiple regression models at once and to fix some syntax errors in Problem 3 part c.
